...
class HierarchyViewModel(QObject):
    ...
    def paste_geometries(self, parent=None):
        ...
        # 已存在名称集合（用于生成不冲突的名称）
        used_names = set(g.name for g in self._find_all_geometries(self.geometries))
        ...
        for item in self._clipboard_items:
            ...
            # 将新名字记录到已用集合
            used_names.add(new_geo.name)

            # 递归复制子对象
            if hasattr(item, 'children') and item.children:
                self._copy_children_recursive(item, new_geo, used_names)
    ...
    def _copy_children_recursive(self, source, target, used_names=None):
        """
        递归复制子对象
        ...
        for child in source.children:
            ...
            # 递归复制子对象
            if hasattr(child, 'children') and child.children:
                self._copy_children_recursive(child, new_child, used_names)

    def _generate_copy_name(self, original_name, used_names):
        """根据原始名称生成不重复的复制名称，如: name_copy_0, name_copy_1 ...

        规则:
        - 若原名已包含 "_copy" 或 "_copy_数字" 后缀，则以去掉该后缀的基名为基础继续编号。
        - 从 0 开始寻找最小可用编号。
        """
        base = original_name
        # 移除已有的 _copy 或 _copy_数字 后缀
        import re
        m = re.match(r"^(.*?)(?:_copy(?:_\d+)?)$", original_name)
        if m:
            base = m.group(1)

        i = 1
        while True:
            candidate = f"{base}_copy_{i}"
    ...
    def reparent_geometry(self, geometry, new_parent):
        ...
        if new_parent:
            ...
            new_source = getattr(new_parent, 'source_file', None)
            if new_source:
                if hasattr(self._scene_viewmodel, '_set_source_recursive'):
                    self._scene_viewmodel._set_source_recursive(geometry, new_source)
                else:
                    geometry.source_file = new_source
        else:
            self._scene_viewmodel.geometries.append(geometry)
            geometry.parent = None
            if hasattr(self._scene_viewmodel, '_set_source_recursive'):
                self._scene_viewmodel._set_source_recursive(geometry, None)
