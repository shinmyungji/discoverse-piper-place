...
class SmartDecimalsSpinBox(QDoubleSpinBox):
    ...
    """
    def __init__(self, max_decimals=15, pretty_decimals=3, parent=None, **kwargs):
        self._fmt_full = "{:.15g}"                   
        self._pretty_decimals = int(pretty_decimals)
        self._editing = False

        super().__init__(parent)
        self.setDecimals(int(max_decimals))          
    ...
    def textFromValue(self, value: float) -> str:
        try:
            ...
            return str(value)
        if self._editing:
            return self._fmt_full.format(v)                    
        return f"{v:.{max(0, self._pretty_decimals)}f}"
...
class PropertyView(QWidget):
    ...
    def _create_joint_group(self):
        ...
        joint_axis_editor = QLineEdit()
        joint_axis_editor.setPlaceholderText("输入 axis (可留空)")
        joint_axis_editor.editingFinished.connect(
            lambda e=joint_axis_editor: self.propertyChanged.emit("joint_axis", e.text().strip())
        )
        joint_layout.addRow("轴 axis:", joint_axis_editor)
        self._joint_axis_edit = joint_axis_editor
    ...
    def _update_ui(self):
        ...
        show_physics = geo_type not in (GeometryType.JOINT.value, "group")
        for key, editor in self._physics_editors.items():
            label = self._physics_layout.labelForField(editor)
            editor.blockSignals(True)
            value = physics_attrs.get(key, "")
        ...
        self._physics_group.setVisible(show_physics)
        ...
        if is_joint:
            ...
            axis_text = self._property_viewmodel.get_property("joint_axis") or ""
            self._joint_axis_edit.blockSignals(True)
            self._joint_axis_edit.setText(axis_text)
            self._joint_axis_edit.blockSignals(False)
