...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, 
                          QGroupBox, QRadioButton, QComboBox, QLabel, QDoubleSpinBox,QFormLayout,
...
class ControlPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        lbl = QLabel("全局大小")
        self.gizmoGlobalSpin = SmartDecimalsSpinBox(max_decimals=15, parent=self)
        self.gizmoGlobalSpin.setRange(0.01, 1000.0)
        self.gizmoGlobalSpin.setSingleStep(0.1)
        gizmo_layout.addWidget(lbl)
        gizmo_layout.addWidget(self.gizmoGlobalSpin, 1)  # 伸缩因子=1
        ...
        self.gs_reset_button = QPushButton("清除 PLY")
        btn_apply.clicked.connect(self._emit_apply)
        self.gs_reset_button.clicked.connect(self._clear_gs_backgrounds)
        row_btn = QHBoxLayout(); row_btn.addWidget(btn_apply); row_btn.addWidget(self.gs_reset_button)
    ...
    def set_gs_ply_entries(self, entries, emit_change=True):
        ...
        if self.gs_file_combo.count() > 0:
            ...
            if emit_change and current_key == selected_key:
    ...
    def _clear_gs_backgrounds(self):
        self._reset_fields()
        if hasattr(self._control_viewmodel, 'clear_gs_backgrounds'):
            self._control_viewmodel.clear_gs_backgrounds()
